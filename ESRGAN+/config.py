...
NUM_WORKERS = 8
LEARNING_RATE = 1e-4
comment = 'RRDB23+WGAN+gram+pixelshuffle'
EPOCHS = 1000
last_EPOCH = 0
lambda_pixel = 0.1
lambda_adv = 5e-3
...
def loader_model(gen,disc):
    if generator_path != '':
        if os.path.exists(generator_path):
            gen.load_state_dict(torch.load(generator_path), strict=True)
        else:
            raise FileNotFoundError("not found weights file: {}".format(generator_path))
